...
<Code code={`WITH recent_orders AS (
    SELECT DISTINCT user_id
    FROM orders
    WHERE order_date > '2025-01-01'
),
...
<Code code={`WITH quarterly_sales AS (
    SELECT
        DATE_TRUNC('quarter', sale_date) AS quarter,
        SUM(sale_amount) AS total_sales
    FROM sales
...
**SQL Server:**
<Code code={`SET STATISTICS TIME ON;
...
<Code code={`WITH quarterly_sales AS MATERIALIZED (
    SELECT 
        DATE_TRUNC('quarter', sale_date) AS quarter,
        SUM(amount) as total
    FROM sales
...
**Force inlining:**
<Code code={`WITH recent_data AS NOT MATERIALIZED (
    SELECT * 
    FROM logs 
...
<Code code={`def get_all_descendants(category_id):
    descendants = []
...
<Code code={`def traverse_org_chart(employee_id, visited=None):
    if visited is None:
        visited = set()
...
<Code code={`CREATE TABLE categories (
    id INT PRIMARY KEY,
    name VARCHAR(100),
...
<Code code={`WITH RECURSIVE cte_name AS (
...
**Database schema:**
<Code code={`CREATE TABLE employees (
    id INT PRIMARY KEY,
    name VARCHAR(100),
    manager_id INT,
...
<Code code={`WITH RECURSIVE org_hierarchy AS (
...
<Code code={`-- Find all reports under a specific manager
...
**Implementation:**
<Code code={`WITH RECURSIVE category_tree AS (
...
<Code code={`-- Include product counts per category
...
<Code code={`WITH RECURSIVE bom AS (
...
<Code code={`WITH RECURSIVE dir_tree AS (
...
<Code code={`WITH RECURSIVE connections AS (
...
**Always include:**
<Code code={`WITH RECURSIVE tree AS (
...
<Code code={`EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
WITH RECURSIVE org_hierarchy AS (
...
<Code code={`WITH RECURSIVE valid_hierarchy AS (
...
<Code code={`WITH RECURSIVE cycle_check AS (
    SELECT 
        id,
        parent_id,
        ARRAY[id] as path,
...
<Code code={`WITH updated_prices AS (
    UPDATE products
    SET 
        price = price * 1.1,
        updated_at = NOW()
...
<Code code={`from sqlalchemy import select, table, column, func
...
<Code code={`import static org.jooq.impl.DSL.*;
...
<Code code={`from django.db.models import Count, Sum, Q
...
<Code code={`// src/Repository/UserRepository.php
...
<Code code={`package com.example.repository;
...
<Code code={`-- Migration: Create view
...
<Code code={`-- Create materialized view
...
<Code code={`# services/analytics_service.py
...
<Code code={`// ❌ DANGEROUS - SQL Injection vulnerability
\$userId = \$_GET['id'];
...
<Code code={`// ✅ SAFE - Parameterized query
...
<Code code={`-- Always use EXPLAIN for performance verification
...
<Code code={`-- Without index on manager_id
EXPLAIN ANALYZE
...
<Code code={`CREATE INDEX idx_employees_manager_id ON employees(manager_id);
...
<Code code={`-- Instead of calculating metrics in CTE every time
...
<Code code={`user = User.objects.get(id=user_id)
user.name = "Updated Name"
user.save()
`} lang="python" />

✅ **Simple filters and relationships**  
<Code code={`active_users = User.objects.filter(active=True, created_at__gte=last_month)
`} lang="python" />
...
<Code code={`-- PostgreSQL declarative partitioning by date range
CREATE TABLE sales (
    id BIGSERIAL,
...
<Code code={`-- Query automatically uses partition pruning
...
**Example:**
<Code code={`-- ClickHouse query on 1B rows executes in milliseconds
SELECT 
...
**Example:**
<Code code={`-- BigQuery handles petabyte-scale aggregations
...
<Code code={`# Application writes to PostgreSQL (OLTP)
order = Order(
    user_id=user_id,
    total_amount=100.50,
    status='completed'
...
- [jOOQ: Common Table Expressions](https://www.jooq.org/doc/latest/manual/sql-building/sql-statements/with-clause/)
...
<Code code={`WITH cte_name AS MATERIALIZED (
...
<Code code={`WITH RECURSIVE cte AS (
